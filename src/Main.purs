...
import Data.GraphQL.Type (ObjectType, Schema, argument, field, field', id, int, objectType, schema, string) as GraphQL
...
queryType = GraphQL.objectType "Query" (Just "The main query type")
  ...
  where
    hello = GraphQL.field' GraphQL.string Nothing \_ -> do
    ...
    user = GraphQL.field userType Nothing args resolve
      where
        args = { id: GraphQL.argument GraphQL.id Nothing }
...
userType = GraphQL.objectType "User" (Just "A type for a user.")
  ...
  where
    id = GraphQL.field' GraphQL.id Nothing (_.id >>> pure)
    firstName = GraphQL.field' GraphQL.string Nothing (_.firstName >>> pure)
    lastName = GraphQL.field' GraphQL.string Nothing (_.lastName >>> pure)
